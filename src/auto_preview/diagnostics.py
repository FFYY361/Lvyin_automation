...
def _diagnose(error: BaseException) -> _Diagnosis:
    ...
    if isinstance(error, THUFootballPermissionError):
        return _Diagnosis(
            "权限错误",
            (
    ...
    if isinstance(error, (SchemaError, DataConflict)):
        return _Diagnosis(
            "远端赛事数据校验错误",
            (
    ...
    if isinstance(error, THUFootballError):
        return _Diagnosis(
            "赛事数据错误",
    ...
    if isinstance(error, WechatConfigurationError):
        return _Diagnosis(
            "公众号本地配置错误",
            ("确认公众号 AppID/AppSecret 等必需环境变量已配置，但不要输出其值。",),
        )
    if isinstance(error, WechatAuthenticationError):
        return _Diagnosis(
            "公众号认证错误",
            ("检查公众号凭据是否有效，并确认服务器出口 IP 已加入公众号白名单。",),
        )
    if isinstance(error, WechatPermissionError):
        return _Diagnosis(
            "公众号权限错误",
            ("确认公众号类型及账号已开通素材上传和草稿箱接口权限。",),
        )
    if isinstance(error, WechatRateLimited):
        return _Diagnosis(
            "公众号接口限流",
            ("等待限流窗口恢复后再试，避免使用 --override 重复创建草稿。",),
        )
    if isinstance(error, WechatTimeout):
        return _Diagnosis(
            "公众号网络超时",
    ...
    if isinstance(error, (MediaUploadError, DraftWriteError)):
        return _Diagnosis(
            "公众号接口写入错误",
            ("根据微信错误码检查素材、封面、正文和接口权限，再核对 draft.json。",),
        )
    if isinstance(error, WechatArticleError):
        return _Diagnosis(
            "公众号接口错误",
...
def failure_lines(
    ...
    *,
    ...
    if isinstance(error, BatchQueryError):
        child_diagnoses = [
            _diagnose(child) for child in error.failures.values()
        ]
    ...
    if suggestions:
        lines.append("  建议：")
        lines.extend(f"    {index}. {value}" for index, value in enumerate(suggestions, 1))
