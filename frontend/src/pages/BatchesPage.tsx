...
export function BatchesPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="内容管理" title="前瞻批次" description="查看状态、定位缺项并重新查询比赛数据。" />
          ...
          <div className="data-table-wrap">
            <table className="data-table">
              ...
              <tbody>{items.map((batch) => (
                <tr key={batch.id}>
                  <td><strong>{formatDate(batch.preview_date)}</strong><small>#{batch.id}</small></td>
                  <td>{competitionLabels[batch.competition]}</td>
                  <td><Badge tone={statusTone(batch.status)}>{statusLabels[batch.status]}</Badge></td>
                  <td>{batch.missing_fields.length ? <span title={batch.missing_fields.map((item) => labelMissingField(item)).join("、")}>{batch.missing_fields.length} 项待补充</span> : <span className="success-text">完整</span>}</td>
                  <td>{batch.last_error ? <span className="error-summary" title={batch.last_error.message ?? ""}>{batch.last_error.message || batch.last_error.code}</span> : <span className="muted">—</span>}</td>
                  <td>{formatDateTime(batch.updated_at)}</td>
                  <td><div className="row-actions"><Button variant="quiet" loading={refreshing === batch.id} onClick={() => void refresh(batch.id)} title="重新查询"><RefreshCw size={16} /></Button><Link className="icon-link" to={`/batches/${batch.id}`} aria-label="打开批次"><ArrowRight size={17} /></Link></div></td>
                </tr>
              ))}</tbody>
            </table>
          </div>
