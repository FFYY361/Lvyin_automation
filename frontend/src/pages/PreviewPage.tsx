...
export function PreviewPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="文章" title={batch ? `${competitionLabels[batch.competition]} · ${batch.preview_date}` : "文章预览"} description="渲染结果保留为不可变版本，内容变化后需要重新渲染。" actions={<><Link className="button button--quiet" to={`/batches/${batchId}`}><ArrowLeft size={16} />返回批次</Link><Button variant="primary" loading={rendering} onClick={() => void render()}><RefreshCw size={16} />{article ? "重新渲染" : "渲染文章"}</Button></>} />
      ...
      {stale ? <Alert tone="warning"><strong>当前数据已发生变化，文章已过期。</strong><span>以下为最近一次渲染结果，请重新渲染后再用于发布。</span></Alert> : null}
      {!article ? <Panel><EmptyState title="尚未渲染文章" description="可以先渲染带缺项提示的预览，完善内容后再重新渲染。" action={<Button variant="primary" loading={rendering} onClick={() => void render()}>开始渲染</Button>} /></Panel> : (
