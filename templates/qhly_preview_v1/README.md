...
source 必须符合 `schema.json`，未知字段会被拒绝。每场文案位于顶层 `previews`，键为 `主队简称 vs 客队简称`。新生成的 source 使用 `article_file` 引用同目录 `previews/主队简称vs客队简称.md`；Markdown 文件可直接粘贴多段纯文本，以空白行分段，不允许注入 HTML。内联 `article` 仅用于兼容已有示例。天气和人员分别从显式传入的 weather/config 文件加载。模板语法、字段校验和文章目录格式见 [前瞻模板与渲染教程](../../docs/preview/preview_template_tutorial.md)。
