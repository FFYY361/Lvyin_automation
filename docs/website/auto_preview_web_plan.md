...
- `POST /api/auth/register` 使用共享邀请码注册固定角色为 `user` 的账号并自动登录；用户名
...
- 不增加邮箱验证、验证码、复杂组织架构、审批流、OAuth 或企业级审计平台；Stage 7
  增加仅保存在环境变量中的共享邀请码。
...
同时保留 Stage 3 管理员闭环。网站在电脑和手机浏览器中均可完成日常前瞻工作；
真实试运行和生产部署留到 Stage 7，PWA 暂不实施。
...
完整操作手册见 [Stage 7：绿茵宣传部生产化与部署](stages/production_deployment.md)。

### PWA（暂不实施）

- 本阶段不增加 manifest、应用图标或“添加到主屏幕”能力。
...
- PostgreSQL 与整个 Artifact 目录每天联合备份；最多保留 7 份且总量不超过 2GB，
  不执行恢复演练。
- 后端通过 Git clone/fetch 获取明确提交，使用 `/home/xfy/miniconda3` 管理生产环境；
  前端只消费本地构建制品，服务器不安装 Node.js 或 pnpm。
